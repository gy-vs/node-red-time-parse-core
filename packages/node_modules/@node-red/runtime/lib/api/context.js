                ...
                if (key) {
                    ...
                    ctx.get(key,store,function(err, v) {
                        if (opts.keysOnly) {
                            ...
                            } else if ((typedArrayName = util.getTypedArrayName(v))) {
                                result.format = `${typedArrayName.toLowerCase()}[${v.length}]`
                            } else if (typeof v === 'object') {
                                result.keys = Object.keys(v).map(k => {
                                    let childTypedArrayName
                                    if (Array.isArray(v[k])) {
                                        return { key: k, format: `array[${v[k].length}]`, length: v[k].length }
                                    } else if ((childTypedArrayName = util.getTypedArrayName(v[k]))) {
                                        return { key: k, format: `${childTypedArrayName.toLowerCase()}[${v[k].length}]`, length: v[k].length }
