...
RED.utils = (function() {
    ...
    function buildMessageSummaryValue(value) {
        ...
        if (Array.isArray(value)) {
            ...
            } else if (value.hasOwnProperty('type') && value.type === 'typedarray' && value.hasOwnProperty('data')) {
                result = $('<span class="red-ui-debug-msg-object-value red-ui-debug-msg-type-meta"></span>').text((value.subtype||'TypedArray')+'['+value.length+']');
    ...
    function createObjectElement(obj,options) {
        ...
        if (obj && typeof obj === 'object' && obj.hasOwnProperty('type') && obj.hasOwnProperty('data') && ((obj.__enc__ && obj.type === 'set') || (obj.__enc__ && obj.type === 'array') || (obj.__enc__ && obj.type === 'typedarray') || obj.type === 'Buffer')) {
            isArray = true;
            isArrayObject = true;
        }
        if (obj === null || obj === undefined) {
            ...
            if (isArrayObject) {
                ...
                if (obj.__enc__ && obj.type === 'typedarray') {
                    // Preserve the concrete TypedArray name (Float32Array, ...)
                    // in the rendered label
                    typedArraySubtype = obj.subtype || 'TypedArray';
                    type = typedArraySubtype;
                }
            ...
            if (key) {
                var nestedTypeLabel = typeHint;
                if (typedArraySubtype) {
                    nestedTypeLabel = typedArraySubtype+'['+originalLength+']';
                }
                headerHead = $('<span class="red-ui-debug-msg-type-meta"></span>').text(nestedTypeLabel||(type+'['+originalLength+']')).appendTo(entryObj);
            } else {
                ...
                $('<span> ]</span>').appendTo(headerHead);
                if (typedArraySubtype) {
                    $('<span class="red-ui-debug-msg-type-meta"></span>').text(' '+typedArraySubtype+'['+originalLength+']').appendTo(entryObj);
                }
            }
            if (originalLength > 0) {
                ...
                makeExpandable(header,function() {
                    if (!key) {
                        var expandedTypeLabel = typeHint||(type+'['+originalLength+']');
                        if (typedArraySubtype) {
                            expandedTypeLabel = typedArraySubtype+'['+originalLength+']';
                        }
                        headerHead = $('<span class="red-ui-debug-msg-type-meta red-ui-debug-msg-object-type-header"></span>').text(expandedTypeLabel).appendTo(header);
                    }
                    if (type === 'buffer') {
    ...
    function decodeObject(payload,format) {
        if ((format === 'number') && (payload === "NaN")) {
            ...
            payload = -Infinity;
        } else if (format === 'Object' || /^(array|set|map|typedarray)/.test(format) || format === 'boolean' || format === 'number' ) {
