...
/**
 * Checks if the value is a TypedArray (other than a Buffer, which has its own
 * encoding). A TypedArray is serialised element-by-element like a plain object
 * by JSON.stringify, which generates one object key per index and can freeze
 * the Debug sidebar when expanded - see the `type: "typedarray"` encoding below.
 *
 * @param  {any} value
    ...
    try {
        ...
        if (msg.msg instanceof Error || /Error/.test(msg.msg?.__proto__?.name)) {
            ...
            if (/error/i.test(msg.format)) {
                ...
                } else if (isTypedArray(msg.msg)) {
                    // A TypedArray (Float32Array, Int16Array, ...) must not be
                    // stringified key-by-key as that produces one property per
                    // index and can freeze the Debug sidebar when expanded.
                    // Encode it as a truncated, array-like value instead.
                    msg.format = "typedarray["+msg.msg.length+"]";
                    msg.msg = {
                        __enc__: true,
                        type: "typedarray",
                        subtype: constructorName(msg.msg),
                    ...
                    msg.msg = safeJSONStringify(msg.msg, function(key, value) {
                        if (key === '_req' || key === '_res') {
                            ...
                            if (value.type === "Buffer") {
                                ...
                                // Encode TypedArrays as truncated, array-like
                                // values rather than as objects with one key
                                // per index (which can freeze the Debug sidebar)
                                value = {
                                    __enc__: true,
                                    type: "typedarray",
                                    subtype: constructorName(value),
