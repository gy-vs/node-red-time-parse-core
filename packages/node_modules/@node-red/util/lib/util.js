...
// The TypedArray types that are encoded as array-like values rather than
// generic objects. Buffer is intentionally excluded as it has its own
// encoding. This must be kept in sync with the editor client.
...
/**
 * Returns the constructor name of a TypedArray (such as Float32Array) or
 * null if the value is not a TypedArray.
 * @param  {any} obj
 * @return {String} the TypedArray constructor name, or null
 */
function getTypedArrayName(obj) {
    if (obj) {
        const name = constructorName(obj);
        if (hasOwnProperty.call(TYPED_ARRAY_NAMES, name)) {
            return name;
        }
    }
    return null;
    ...
    try {
        ...
        if (msg.msg instanceof Error || /Error/.test(msg.msg?.__proto__?.name)) {
            ...
            if (/error/i.test(msg.format)) {
                ...
                } else if ((typedArrayName = getTypedArrayName(msg.msg))) {
                    // Encode TypedArrays as array-like values so they can be
                    // paged in the sidebar instead of having every indexed
                    // key rendered as a standalone object row
                    msg.format = typedArrayName.toLowerCase()+"["+msg.msg.length+"]";
                    msg.msg = {
                        __enc__: true,
                        type: "typedarray",
                        subtype: typedArrayName.toLowerCase(),
                        data: Array.prototype.slice.call(msg.msg, 0, debuglength),
                        length: msg.msg.length
                    }
                    needsStringify = true;
                ...
                if (needsStringify || (msg.format === "Object")) {
                    msg.msg = safeJSONStringify(msg.msg, function(key, value) {
                        if (key === '_req' || key === '_res') {
                            ...
                            } else if ((typedArrayName = getTypedArrayName(value))) {
                                value = {
                                    __enc__: true,
                                    type: "typedarray",
                                    subtype: typedArrayName.toLowerCase(),
                                    data: Array.prototype.slice.call(value, 0, debuglength),
                                    length: value.length
                                }
...
module.exports = {
    ...
    getTypedArrayName: getTypedArrayName
};
